...
- 覆盖率基线（Go/TS）未完成：当前 CI 未设置覆盖率阈值门禁，`apps/web` 仍为 test placeholder。
- Web 控制台面板未完成：`apps/web` 当前仅 Chat 页面，尚未实现 Models/Envs/Skills/Workspace/Cron 最小操作面板。
- 前端错误处理规范未完成：现有状态栏提示可用，但缺少统一错误展示规范文档与对应测试约束。
- 前端测试未完成：`apps/web` 尚未接入单测与冒烟测试用例。
